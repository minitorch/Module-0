...
def test_sum_distribute(ls1, ls2):
    ...
    Args:
        ls1 (list[numerical]): value
        ls2 (list[numerical]): value
    """
    a = sum(ls1) + sum(ls2)  # sum lists individually, then add
    b = sum(ls1 + ls2)  # concat the lists then sum all elements
    assert_close(a, b)
