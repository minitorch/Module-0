...
@pytest.mark.task0_2
@given(st.floats(width=16, allow_infinity=False, allow_nan=False))
...
def test_transitive(a: float, b: float, c: float) -> None:
    ...
    assert (
        ...
        and lt(b, c) == 1.0
        and lt(a, c) == 1.0
        or not (lt(a, b) == 1.0 and lt(b, c) == 1.0)
    )
...
def test_other() -> None:
    ...
    assert_close(inv(inv(4.0)), 4.0)
    assert_close(neg(neg(5.0)), 5.0)
    assert_close(add(0.0, 7.0), 7.0)
    assert_close(mul(1.0, 8.0), 8.0)
